...
class OwnerTuitionReceiptTests(unittest.TestCase):
    ...
    def test_durable_checkpoint_is_exactly_first_three(self):
        self.assertEqual(list(self.by_row), [164, 178, 193])
        self.assertEqual(self.data["pending_in_this_batch"], ["Jen Rollins $618 on 2026-06-26"])
        self.assertEqual(
            sum((Decimal(item["amount"]) for item in self.data["transactions"]), Decimal("0")),
            Decimal("2015.00"),
        )

    def test_all_three_have_direct_sales_treatment(self):
    ...
    def test_queue_and_balance_controls(self):
        ...
        after = self.data["controls"]["after_first_three"]
        self.assertEqual(before["unreconciled_statement_lines"], 44)
        self.assertEqual(after["unreconciled_statement_lines"], 41)
        self.assertEqual(after["fy2025_26_unreconciled_lines"], 4)
        ...
        self.assertEqual(Decimal(after["book_balance"]), Decimal("29121.75"))
        self.assertEqual(
