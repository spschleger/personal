...
- Xero reconstruction is in progress. Of 253 imported account `1913` statement lines, 108 are reconciled and 145 remain (42.7% complete). Completed statement-line work comprises 49 source-supported invoice-related receipts, nine Digital Pacific hosting expenses totalling $162.70, all 27 Microsoft-related lines, all nine Xero subscription debits totalling $315, all 13 office-rent debits totalling $9,750, and the $2,000 transfer to account `9316` posted to `880 - Owner A Drawings`.
- Stripe reconstruction has started. `091 - Stripe Clearing` now exists in Xero as an AUD `Other` account with a zero opening balance and `BAS Excluded` treatment. `508 - Stripe Fees` was corrected from `GST on Expenses` to `BAS Excluded`. INV-0043's verified $665 Stripe payment was removed from `090 - Business Account` and re-recorded to `091 - Stripe Clearing` on 2 March 2026 under charge reference `ch_3T6TAbS4D20Qoe6M0X0u1Nyc`. Five invoice payments totalling $4,310, six fees totalling $105.89 and six owner-personal payouts totalling $4,869.11 remain to be posted; the completed chain must return Stripe Clearing to zero.
...
- Three bank rules are saved and visibly confirmed in Xero: `Microsoft subscriptions` (bank text contains `Microsoft`; contact `Microsoft`; `485 - Subscriptions`; `BAS Excluded`), `Xero subscriptions` (description contains `XERO AU`; contact `Xero`; `485 - Subscriptions`; `BAS Excluded`), and `Office rent` (description contains `Rent`; contact `PTC Trust`; `469 - Rent`; `BAS Excluded`). Before the first Stripe payment reclassification, Xero showed statement balance $31,016.85, book balance $25,604.15 and a $5,412.70 difference. That displayed balance checkpoint is now historical and must be refreshed before being quoted as current.
...
- Immediate sequence: finish the five remaining Xero-linked invoice-payment moves to `091 - Stripe Clearing`; move all six Stripe fees to `508 - Stripe Fees`; post the six net payouts to account `9316` as owner-personal movements; and prove Stripe Clearing closes to zero. Then resolve the remaining unmatched credits and classify the other evidence-led supplier debits, owner movements and ambiguous withdrawals. Receipt-supported business expenses paid personally will be entered later as owner-funded expenses, after invoices and income are complete.
