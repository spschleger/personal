...
- Xero reconstruction is in progress. Of 253 imported account `1913` statement lines, 89 are reconciled and 164 remain. Completed statement-line work comprises 49 source-supported invoice-related receipts, nine Digital Pacific hosting expenses totalling $162.70, all 27 Microsoft-related lines, and four of nine Xero subscription debits. The Microsoft source set—26 debits totalling $283.09 and one $1 refund—is complete. Five Xero subscription debits totalling $175 remain.
...
- A `Microsoft subscriptions` bank rule is saved in Xero: bank text contains `Microsoft`; contact `Microsoft`; account `485 - Subscriptions`; description `Microsoft software subscription`; tax rate `BAS Excluded`. A second saved rule, `Xero subscriptions`, matches description containing `XERO AU`, uses contact `Xero`, account `485 - Subscriptions`, description `Xero subscription` and tax rate `BAS Excluded`. At the latest verified checkpoint, Xero showed statement balance $31,016.85, book balance $37,529.15 and a $6,512.30 difference, so the account is not reconciled.
- Immediate sequence: finish five Xero subscription debits totalling $175, then 13 office-rent debits totalling $9,750. That should reduce the queue to 146 before the account `9316` / Stripe-clearing work and the remaining evidence-led classifications.
