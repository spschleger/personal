...
- Xero reconstruction is in progress. Of 253 imported account `1913` statement lines, 143 are reconciled and 110 remain (56.5% complete). The latest bounded run added 25 FY2025–26 Transport for NSW debits totalling $185.23 as `Spend money` to existing account `493 - Travel - National`, contact `Transport for NSW`, description `Business travel - Transport NSW`, and `BAS Excluded`. Exact date/amount read-back and queue controls are in `workpapers/2026-09-07-xero-transport-expense-batch.json`.
